...
describe('lesson.service', () => {
    ...
    describe('getLessons', () => {
        it('returns lessons sorted by order', async () => {
            ...
            Lesson.find.mockReturnValue({ sort: sortMock });
    ...
    describe('createLesson', () => {
        it('assigns order 0 when no existing lessons', async () => {
            ...
            Lesson.findOne.mockReturnValue({ sort: sortMock });
            Lesson.create.mockResolvedValue({ ...mockLesson, order: 0 });
        ...
        it('assigns order as last.order + 1 when lessons exist', async () => {
            ...
            Lesson.findOne.mockReturnValue({ sort: sortMock });
            Lesson.create.mockResolvedValue({ ...mockLesson, order: 3 });
            const result = await lessonService.createLesson({
                moduleId: 'module-id-1',
                title: 'New Lesson',
            });
            expect(Lesson.create).toHaveBeenCalledWith(expect.objectContaining({ order: 3 }));
        ...
        it('creates lesson with empty contents array', async () => {
            ...
            expect(Lesson.create).toHaveBeenCalledWith(expect.objectContaining({ contents: [] }));
        });
    });
    describe('updateLesson', () => {
        it('returns updated lesson', async () => {
            ...
            expect(Lesson.findByIdAndUpdate).toHaveBeenCalledWith('lesson-id-1', { title: 'Updated' }, { new: true });
        ...
        it('can update contents array', async () => {
            const contents = [{ _id: 'block-1', type: 'text', body: 'Hello' }];
            const updated = { ...mockLesson, contents };
            Lesson.findByIdAndUpdate.mockResolvedValue(updated);
            const result = await lessonService.updateLesson('lesson-id-1', { contents });
            expect(Lesson.findByIdAndUpdate).toHaveBeenCalledWith('lesson-id-1', { contents }, { new: true });
            expect(result).toEqual(updated);
        });
    });
