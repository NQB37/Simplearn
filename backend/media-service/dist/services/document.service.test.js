...
describe('Document Service', () => {
    ...
    describe('uploadDocument', () => {
        it('should upload document and return metadata', async () => {
            ...
            const result = await documentService.uploadDocument(fakeBuffer, originalName, mimetype, userId);
            expect(supabase.storage.from).toHaveBeenCalledWith('documents');
            expect(mockUpload).toHaveBeenCalledWith(expect.stringMatching(new RegExp(`^${userId}/\\d+-${originalName}$`)), fakeBuffer, { contentType: mimetype });
        ...
        it('should throw when supabase returns an error', async () => {
            ...
            await expect(documentService.uploadDocument(buffer, 'file.pdf', 'application/pdf', 'user-456')).rejects.toThrow('Storage quota exceeded');
        });
    });
    describe('deleteDocument', () => {
        ...
        it('should throw when supabase returns an error', async () => {
            ...
            await expect(documentService.deleteDocument('user-123/nonexistent.pdf')).rejects.toThrow('File not found');
        });
    });
    describe('getDocumentUrl', () => {
        ...
        it('should throw when supabase returns an error', async () => {
            ...
            await expect(documentService.getDocumentUrl('user-123/missing.pdf')).rejects.toThrow('Path not found');
        });
    });
});
