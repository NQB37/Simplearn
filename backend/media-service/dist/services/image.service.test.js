...
describe('Image Service', () => {
    ...
    describe('uploadImage', () => {
        it('should resolve with image data on successful upload', async () => {
            ...
            expect(cloudinary.uploader.upload_stream).toHaveBeenCalledWith({ folder: 'simplearn', resource_type: 'image' }, expect.any(Function));
        ...
        it('should reject when cloudinary returns an error', async () => {
            ...
            const buffer = Buffer.from('fake-image-data');
            await expect(imageService.uploadImage(buffer, 'image/jpeg')).rejects.toThrow('Cloudinary upload failed');
        });
    ...
    describe('deleteImage', () => {
        ...
        it('should throw when image is not found', async () => {
            ...
            await expect(imageService.deleteImage('simplearn/nonexistent')).rejects.toThrow('Image not found');
        });
    });
});
