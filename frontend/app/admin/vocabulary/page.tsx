...
import { useFaculties, useMajors, useVocabularyMutations } from '@/hooks/use-user';
...
export default function VocabularyPage() {
  ...
  const { createFacultyMutation, deleteFacultyMutation, createMajorMutation, deleteMajorMutation } = useVocabularyMutations();
  ...
  const handleAddMajor = () => {
    ...
    createMajorMutation.mutate({ name: newMajorName, facultyId: selectedFieldId }, {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <CardContent className="space-y-4">
            <div className="flex gap-2">
              ...
              <Button
                ...
                disabled={createFacultyMutation.isPending || !newFieldName.trim()}
              >
                {createFacultyMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin" /> : <Plus className="h-4 w-4 mr-2" />}
                Add
              </Button>
            </div>
            ...
            <div className="space-y-2 max-h-[400px] overflow-auto">
              {faculties?.map((faculty) => (
                <div key={faculty._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
              ...
              {faculties?.length === 0 && <p className="text-sm text-muted-foreground text-center py-4">No faculties added yet.</p>}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="rounded-2xl shadow-sm border-slate-200 dark:border-slate-800 bg-white dark:bg-slate-900">
          <CardHeader>
            ...
            <CardDescription>Specific programs within a faculty.</CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
                ...
                <div key={major._id} className="flex items-center justify-between p-2 rounded-lg hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors">
                  <div className="flex flex-col">
                    ...
                    <span className="text-xs text-muted-foreground">
                      {faculties?.find(f => f._id === major.facultyId)?.name || 'Unknown Faculty'}
                    </span>
                  </div>
                  <Button
                    variant="ghost"
