...
import { useMajors } from '@/hooks/use-user';
import { useCurriculum, useCurriculumMutations, useSubjects } from '@/hooks/use-academics';
import { Semester, MajorSubject, Subject } from '@/types/academics.type';
...
function AddSubjectDialog({ majorId, studyYear, semester, open, onClose }: AddSubjectDialogProps) {
  ...
  const handleAdd = () => {
    ...
    addMutation.mutate(
      { majorId, subjectId: selectedSubjectId, studyYear, semester, isMandatory },
      {
        onSuccess: () => {
...
function SemesterBlock({ majorId, studyYear, semester, semesterLabel, entries }: SemesterBlockProps) {
  ...
  return (
    <div className='border border-slate-200 dark:border-slate-700 rounded-lg p-4 flex flex-col gap-3 bg-white dark:bg-slate-900'>
      <h3 className='text-sm font-semibold text-slate-700 dark:text-slate-300'>{semesterLabel}</h3>
...
export default function CurriculumPage() {
  ...
  const { data: curriculum = [], isLoading: curriculumLoading } = useCurriculum(selectedMajorId);
  ...
  return (
    <div className='max-w-7xl mx-auto space-y-8'>
      ...
      <div className='flex items-center gap-4'>
        ...
        >
          <SelectTrigger className='w-[280px]'>
            <SelectValue placeholder={majorsLoading ? 'Loading...' : 'Select a major'} />
          </SelectTrigger>
          <SelectContent>
            {majors.map((m) => (
      ...
      {!selectedMajorId && !majorsLoading && (
        <p className='text-slate-400'>Select a major above to view and edit its curriculum.</p>
      )}
    </div>
  );
}
