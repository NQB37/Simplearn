...
export function EditSubjectModal({ subject }: EditSubjectModalProps) {
  ...
  const { data: filteredMajors = [] } = useMajors(
  ...
  useEffect(() => {
    if (subject && isOpen) {
      const curriculumEntry = subject.curriculum?.[0];
      const resolvedMajorId = curriculumEntry?.majorId ?? '';
      const resolvedFacultyId = resolvedMajorId
        ? (allMajors.find((m) => m._id === resolvedMajorId)?.facultyId ??
          '')
        : '';
      ...
      setSubjectForm({
        ...
        studyYear: curriculumEntry?.studyYear ?? '',
        semester: curriculumEntry?.semester ?? '',
      });
    }
  }, [subject, isOpen, allMajors]);
  ...
  const handleSave = async () => {
    ...
    const payload: Parameters<typeof updateMutation.mutate>[0]['payload'] = {
          ...
          <div className='flex items-center gap-3'>
            ...
            <div>
              ...
              <p className='text-xs text-zinc-500'>
                Update subject details in the catalog
              </p>
            </div>
          </div>
          <button
